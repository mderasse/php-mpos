        ...
        <table class="table table-striped table-bordered table-hover">
          <thead>
            <tr>
              ...
              <th class="text-right">{t currency=$GLOBAL.config.currency}%1/Day{/t}</th>
              {if $GLOBAL.config.price.enabled}
              {if $GLOBAL.config.price.currency}<th class="text-right">{t currency_price=$GLOBAL.config.price.currency}%1/Day{/t}</th>{/if}
               {/if}
            </tr>
          </thead>
          <tbody>
...
{section contrib $CONTRIBHASHES}
      {math assign="estday" equation="round(reward / ( diff * pow(2,32) / ( hashrate * 1000 ) / 3600 / 24), 3)" diff=$DIFFICULTY reward=$REWARD hashrate=$CONTRIBHASHES[contrib].hashrate}
            {if $GLOBAL.userdata.username|default:""|lower == $CONTRIBHASHES[contrib].account|lower}{assign var=listed value=1}<tr class="success">{else}<tr>{/if}
              ...
              <td>{if $CONTRIBHASHES[contrib].is_anonymous|default:"0" == 1 && $GLOBAL.userdata.is_admin|default:"0" == 0}{t}anonymous{/t}{else}{$CONTRIBHASHES[contrib].account|escape}{/if}</td>
