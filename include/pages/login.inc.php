...
if ($user->isAuthenticated(false)) {
  ...
  isset($_SESSION['USERDATA']['has_twofa']) &&
  $_SESSION['USERDATA']['has_twofa'] &&
  !$_SESSION['USERDATA']['validate_twofa']
) {
  if (!empty($_POST['otp'])) {
    if ($user->isValidTwofa($_POST['otp'])) {
      $user->validateTwofaLogin();
      $port = ($_SERVER["SERVER_PORT"] == "80" || $_SERVER["SERVER_PORT"] == "443") ? "" : (":" . $_SERVER["SERVER_PORT"]);
      $location = (@$_SERVER['HTTPS'] == "on") ? 'https://' : 'http://';
      $location .= $_SERVER['HTTP_HOST'] . $port . $_SERVER['SCRIPT_NAME'];
      $location .= '?page=dashboard';
      ...
      $_SESSION['POPUP'][] = array('CONTENT' => $twofa->getErrorWithDescriptionHTML(), 'TYPE' => 'alert alert-warning');
      $smarty->assign('CONTENT', 'twofa.tpl');
    }
  } else {
    $smarty->assign('CONTENT', 'twofa.tpl');
  }
      ...
      if (($recaptcha_enabled && $rsp->isSuccess()) || !$recaptcha_enabled) {
        if (!$config['csrf']['enabled'] || $config['csrf']['enabled'] && $csrftoken->valid) {
          ...
          if ($user->checkLogin(@$_POST['username'], @$_POST['password'])) {
            ...
            $location = (@$_SERVER['HTTPS'] == "on") ? 'https://' : 'http://';
            $location .= $_SERVER['HTTP_HOST'] . $port . $_SERVER['SCRIPT_NAME'];
            if ($_SESSION['USERDATA']['has_twofa']) {
              $location .= '?page=login';
            } else {
              $location .= '?page=dashboard';
            }
