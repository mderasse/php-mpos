  ...
  public function sendMail($template, $aData, $throttle = false)
  {
    ...
    if ($this->config['swiftmailer']['type'] == 'smtp' && $throttle) {
      $mailer->registerPlugin(new Swift_Plugins_ThrottlerPlugin(
        $this->config['swiftmailer']['smtp']['throttle'],
        Swift_Plugins_ThrottlerPlugin::MESSAGES_PER_MINUTE
      ));
    }

    // Prepare the smarty templates used
    $this->smarty->clearCache(TEMPLATE_DIR . '/mail/global/header.tpl');
    ...
    $message = Swift_Message::newInstance()
      ...
      ->setBody(
        $this->smarty->fetch(TEMPLATE_DIR . '/mail/global/header-plaintext.tpl') . "\n" .
          $this->smarty->fetch(TEMPLATE_DIR . '/mail/' . $template . '-plaintext.tpl') . "\n" .
          $this->smarty->fetch(TEMPLATE_DIR . '/mail/global/footer-plaintext.tpl')
      )
      ->addPart(
        $this->smarty->fetch(TEMPLATE_DIR . '/mail/global/header-html.tpl') . "\n" .
          $this->smarty->fetch(TEMPLATE_DIR . '/mail/' . $template . '-html.tpl') . "\n" .
          $this->smarty->fetch(TEMPLATE_DIR . '/mail/global/footer-html.tpl'),
        'text/html'
      );
    if (
      isset($aData['senderName']) &&
